    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            while (playGameNow)
            {
                ...
                Console.WriteLine("Attempt #: {0} of {1}", attempts, NUMBER_OF_GUESS_ATTEMPTS);
                ...
                for (int c = 0; c < NUMBER_OF_DIGITS_IN_NUMBER; c++)
                {
                    ...
                    if (positionCorrect == NUMBER_OF_DIGITS_IN_NUMBER)
                    {
                        ...
                        Console.WriteLine(" You correctly selected the number: {0} in {1} of {2} possible guesses.", targetNumber, attempts, NUMBER_OF_GUESS_ATTEMPTS);
                        Console.WriteLine(" ");
                        Console.ReadKey();
                        playGameNow = false;
                    }
                ...
                if ((attempts >= NUMBER_OF_GUESS_ATTEMPTS) && (playGameNow == true))
                {
                    ...
                    Console.WriteLine("You failed to select the number: {0} in the number of alloted guesses.", targetNumber);
                    Console.WriteLine(" ");
                    Console.ReadKey();
                    playGameNow = false;
                }
        ...
        {
            Console.WriteLine("The following error occurred: {0}. Please make sure you entered a valid number. Program will end now.", ex.Message);
            Console.ReadKey();
